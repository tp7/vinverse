...
static void mt_makediff_avx2(uint8_t* dstp, const uint8_t* c1p, const uint8_t* c2p, int dst_pitch, int c1_pitch, int c2_pitch, int width, int height)
{
    int mod32_width = (width + 31) & ~31;
...
{
    vertical_blur3_avx2(tempp, srcp, temp_pitch, src_pitch, width, height); //temp = rg11
    mt_makediff_avx2(dstp, srcp, tempp, dst_pitch, src_pitch, temp_pitch, width, height); //dst = rg11D
    vertical_blur3_avx2(tempp, dstp, temp_pitch, dst_pitch, width, height); //temp = rg11D.vblur()
